...
public class TelegramNotifyVelocity {
    ...
    public TelegramNotifyVelocity(ProxyServer server, Logger logger) {
        this.server = server;
        this.logger = logger;
        this.telegramSender = new TelegramSender(); // Убрали FileConfiguration
    ...
    public void onPlayerJoin(PostLoginEvent event) {
        ...
        telegramSender.sendMessage("Игрок зашел: " + player.getUsername() + " (IP: " + player.getRemoteAddress() + ")");
    }

    @Subscribe
    public void onPlayerLeave(DisconnectEvent event) {
        Player player = event.getPlayer();
        telegramSender.sendMessage("Игрок вышел: " + player.getUsername() + " (IP: " + player.getRemoteAddress() + ")");
    }
}